...
export default function Profile() {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const updated = await api('/users/me', { method: 'PATCH', body: JSON.stringify({ nickname }) });
      setUser(updated);
      setEditing(false);
  ...
  const menuItems = [
    ...
    { label: '深色模式', icon: 'M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z', path: '' },
    ...
    <div className="h-full flex flex-col bg-bg page-enter">
      ...
      <div className="bg-white px-5 pt-14 pb-6">
        ...
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-400 to-primary flex items-center justify-center text-white text-xl font-bold shadow-lg shadow-primary/20">
            {(user?.nickname || 'U')[0]}
          </div>
      ...
      <p className="text-center text-xs text-text-secondary/30 mt-auto pb-4">ChatApp v1.0.0</p>
    </div>
  );
}

...
export default function Search() {
  ...
  const handleAddFriend = async (userId: string) => {
    try { await friendApi.add(userId); } catch {}
  };
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {!loading && results.map(u => (
          <div key={u.id} className="flex items-center bg-white border-b border-border px-5 py-3">
            ...
            <button onClick={() => handleAddFriend(u.id)} className="px-3 py-1.5 bg-primary/10 text-primary rounded-lg text-xs font-medium">
              添加
            </button>
          </div>
        ))}

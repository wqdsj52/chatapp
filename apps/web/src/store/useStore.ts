...
interface AppState {
  ...
  messages: Record<string, Message[]>; notifications: Notification[]; friends: User[];
...
export const useStore = create<AppState>((set, get) => ({
  ...
  sessions: [], currentSessionId: null, messages: {}, notifications: [], friends: [],
  ...
  logout: () => { localStorage.removeItem('token'); localStorage.removeItem('user'); disconnectSocket(); set({ token: null, user: null, sessions: [], messages: {}, notifications: [], friends: [] }); },

  fetchProfile: async () => { try { const user = await userApi.getMe(); set({ user }); localStorage.setItem('user', JSON.stringify(user)); } catch { get().logout(); } },
  fetchSessions: async () => { const sessions = await chatApi.getSessions(); set({ sessions }); },
  ...
  fetchFriends: async () => { try { const friends = await friendApi.getAll(); set({ friends }); } catch {} },
  ...
  uploadAvatar: async (file) => { const res = await userApi.uploadAvatar(file); await get().fetchProfile(); return res; },
  ...
  initSocket: () => {
    ...
    socket.on('message:new', (msg) => get().addIncomingMessage(msg));
